...
THEMES = {
    ...
    "frost": {
        ...
        "deco": ('.cd-frost{position:fixed;inset:0;width:100%;height:100%;pointer-events:none;z-index:2;opacity:.85;will-change:opacity;transform:translateZ(0);backface-visibility:hidden;animation:cd-frost-form 48s ease-in-out infinite}@keyframes cd-frost-form{0%{opacity:.10}42%{opacity:.85}58%{opacity:.85}100%{opacity:.10}}@media (prefers-reduced-motion:reduce){.cd-frost{animation:none;opacity:.8}}'),
        "deco_html": ('<svg class="cd-frost" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none"><defs><filter id="cdfrostf" x="-10%" y="-10%" width="120%" height="120%" color-interpolation-filters="sRGB"><feTurbulence type="fractalNoise" baseFrequency="0.012 0.34" numOctaves="3" seed="11" result="n"/><feColorMatrix in="n" type="matrix" result="c" values="0 0 0 0 0.80  0 0 0 0 0.92  0 0 0 0 1.00  0 0 0 -1.65 1.02"/><feGaussianBlur in="c" stdDeviation="0.35"/></filter><radialGradient id="cdfrostm" cx="50%" cy="50%" r="78%"><stop offset="30%" stop-color="#000"/><stop offset="72%" stop-color="#777"/><stop offset="100%" stop-color="#fff"/></radialGradient><mask id="cdfrostmask"><rect width="100%" height="100%" fill="url(#cdfrostm)"/></mask></defs><g mask="url(#cdfrostmask)"><rect class="cd-frost-a" width="100%" height="100%" filter="url(#cdfrostf)"/></g></svg>'),
