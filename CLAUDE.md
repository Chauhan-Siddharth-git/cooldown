...
|---|---|
| **Gated-site origin** (`https://www.reddit.com/budget/*`) | The gate, the session endpoints, `/feed`, `/worth`. **Six endpoints against a cap of 8** (cap set 2026-08-02 with the set at 7; `/worth` took the last slot on 08-03; `/study` and `/exit` left with study mode on 09-16, the first time the number has gone *down*). Two slots free is not permission to spend them — the cap exists because any script on that site, including its ads, is same-origin with all of it. |
